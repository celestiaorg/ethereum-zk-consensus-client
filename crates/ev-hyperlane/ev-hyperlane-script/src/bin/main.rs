...
use ev_zkevm_types::programs::hyperlane::{
    tree::MerkleTree,
    types::{
        HyperlaneBranchProof, HyperlaneBranchProofInputs, HyperlaneMessageInputs,
        HyperlaneMessageOutputs, HYPERLANE_MERKLE_TREE_KEYS,
    },
};
    ...
    if args.execute {
        ...
        let proof_outputs: HyperlaneMessageOutputs =
            bincode::deserialize(proof.public_values.as_slice())
                .expect("Failed to deserialize proof outputs");
...
async fn write_proof_inputs(stdin: &mut SP1Stdin, args: &Args) -> Result<()> {
    ...
    let proof = provider
        ...
        .block_id(alloy_eips::BlockId::Number(
            alloy_eips::BlockNumberOrTag::Number(args.target_height.into()),
        ))
        .await
